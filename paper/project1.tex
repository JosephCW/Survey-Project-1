...
\end{figure}
	\section{Conclusions}
	As cleary shown through our graph, Figure~\ref{fig:time-graph}, 
	and time table, Figure~\ref{fig:time-table}, 
	Kadane's algorithm is a much more optimized solution to solve the problem. 
	A brute forcing algorithm can be simplified down to $O(n^2)$, but our algorithm runs in $O(n^3)$. 
	At the same time Kadane's algorithm runs in $O(n)$ time meaning that it will execute much faster. The amount of time that this algorithm saves is exponential.
	\end{document}
